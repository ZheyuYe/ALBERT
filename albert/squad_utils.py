...
import string
import sys
from albert import modeling, optimization, tokenization, custom_optimization
import numpy as np
import six
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def compute_loss(logits, positions):
                ...
                log_probs = tf.nn.log_softmax(
                    logits, axis=-1)  # (batch_size,seq_length)
                loss = -tf.reduce_mean(
            ...
            def compute_loss_sparse(logits, positions):
                log_probs = tf.nn.sparse_softmax_cross_entropy_with_logits(
                    labels=positions, logits=logits)
                ...
                for end_index in end_indexes:
                    ...
                    if (start_idx, end_idx) not in result_dict[example_index][feature.unique_id]:
                        result_dict[example_index][feature.unique_id][(
                            start_idx, end_idx)] = []
                    result_dict[example_index][feature.unique_id][(
                        start_idx, end_idx)].append((start_log_prob, end_log_prob))
...
def write_predictions_v2(result_dict, cls_dict, all_examples, all_features,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        score_diff = sum(cls_dict[example_index]) / \
            len(cls_dict[example_index])
